      ...
      <div class="col-span-5">
        <div class="flex flex-col gap-3">
          ...
          <div id="edit-avatar" class="flex items-center gap-3 " >
            ...
            <img @click="openFileInput" class="h-20 w-20 rounded-full cursor-pointer" :src="profile.photo" alt="userAvatar">
          ...
          <div class="flex flex-col items-center justify-center gap-3" >
            <form @submit.prevent="updateUsername()" class="flex items-center w-full  gap-3">
              <fieldset class="w-full" >
                ...
                <input type="text" v-model="profile.username" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"  >
              </fieldset>
              <button type="submit" class="mt-8 focus:outline-none text-white bg-red-700 hover:bg-red-800 focus:ring-4 focus:ring-red-300 font-medium rounded-lg text-sm px-5 py-2.5  dark:bg-red-600 dark:hover:bg-red-700 dark:focus:ring-red-900">Submit</button>
            </form>
  ...
  methods: {
    ...
    async handleFileSelect(event) {
      ...
      if (selectedFile) {
        ...
        this.pending = true
        const {data, error} = await useLazyAsyncData('upload-avatar'+Math.random()*100,
            () =>
                $fetch(`${useRuntimeConfig().public.apiUrl}/api/v1/file/image`, {
                  method: 'POST',
                  server: false,
                  headers: {
    ...
    async updatePhoto() {
      this.pending = true
      const {data, error} = await useFetch(`${useRuntimeConfig().public.apiUrl}/api/user/edit-photo`, {
        key: 'update-photo_'+Math.random()*100,
        method: 'PUT',
        headers: {
          Authorization: `Bearer ${useAuthStore().token}`
        },
    ...
    async updateUsername(){
      ...
      const {data, error} = await useFetch(`${useRuntimeConfig().public.apiUrl}/api/user/edit-username`, {
    ...
    async updateFullname(){
      this.pending = true
      const {data, error} = await useFetch(`${useRuntimeConfig().public.apiUrl}/api/user/edit-fullname`, {
