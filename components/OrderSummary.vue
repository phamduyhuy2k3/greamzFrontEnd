<template>
  ...
  <div class="shadow-lg rounded-md border border-neutral-100 ">
    <div class="flex justify-between items-end bg-transparent py-2 px-4 md:px-6 md:pt-6 md:pb-4">
      ...
      <p class="typography-text-base font-medium">(Items: {{ useCart().getCartItemCount }})</p>
    </div>
    <div class="px-4 pb-4 mt-3 md:px-6 md:pb-6 md:mt-0">
      <div class="flex justify-between typography-text-base pb-4">
        ...
        <div class="flex flex-col text-right">
          <p>{{ $currency(useCart().getCartTotal) }}</p>
          <p class="typography-text-xs text-neutral-500">{{ $currency(useCart().getCartTotalOriginal) }}</p>
          <p class="typography-text-xs text-secondary-700">{{ $currency(useCart().getTotalDiscount) }}</p>
          <!--            <p>{{ formatPrice(orderDetails.tax) }}</p>-->
        </div>
      </div>

      <p class="px-3 py-1.5 bg-[#a78bfa] typography-text-sm rounded-md text-center mb-4">
        You are saving {{ $currency(useCart().getTotalDiscount) }} on your order today!
      </p>
      <div
